...
The ``xcom_cfg`` register's tick-config field (``i_cfg_tick``) sets the period of the strobe signal in ``t_clk`` cycles. The relationship is linear (period = 2 × (value + 1)), not a power-of-two doubling, and permitted values range from 0 to 7:
...
.. list-table::
   ...
   * - ``qp_dt1_i[31:0]``
     - Input
     - 32-bit data 1 from processor to peripheral (drives ``AXI_DT1``/``core_data1``)
   * - ``qp_dt2_i[31:0]``
     - Input
     - 32-bit data 2 from processor to peripheral (drives ``AXI_DT2``/``core_data2``)
   * - ``qp_rdy_o``
     - Output
   ...
   * - ``qp_dt1_o[31:0]``
     - Output
     - Data 1 from peripheral to processor
   * - ``qp_dt2_o[31:0]``
     - Output
     - Data 2 from peripheral to processor
   * - ``qp_flag_o``
     - Output
     - Flag for inter-board signaling

The generic QICK peripheral template (``qick_periph.sv``) actually exposes four
32-bit data inputs (``qp_dt1_i``..``qp_dt4_i``) and two 32-bit data outputs
(``qp_dt1_o``, ``qp_dt2_o``); XCOM only uses the first two inputs and both
outputs, matching its two-register (``DT1``/``DT2``) command payload.
