...
.. code-block:: text
   ...
   TEST -op(s_status AND #0x4000)   // Test FIFO_FULL bit (bit 14)
...
If you write ports faster than the dispatcher can process, the FIFO will fill and the core will stall. Monitor `s_status[14]` (FIFO_FULL).
