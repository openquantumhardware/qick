...
This cheatsheet is for the **tProcessor v2** (72-bit instructions, 32-bit registers).
...
.. list-table::
   ...
   * - Wait for division
     - ``WAIT div_dt``
   * - Wait for ARITH (no WAIT macro; poll manually)
     - ``TEST -op(s_status AND bit_arith_new) -uf`` / ``JUMP HERE -if(Z)``
   * - Wait for input data
     - ``WAIT port_dt``
   * - Start division
     - ``DIV r1 r2``
...
.. code-block:: python
   ...
   # 2. Convert to ndarray int32 (N,8) -- must be int32, the assembler
   #    packs each 72-bit instruction into three ints (two 32-bit + one 8-bit)
   buff = np.array(bin_arr, dtype=np.int32)
   # 3. Load in pmem
...
.. code-block:: python
  ...
  def pack_waveform(freq, phase, env, gain, length, conf):
      ...
      wave[2] = env & 0xFFFFFF         # 24-bit envelope index (w_env)
      wave[3] = gain & 0xFFFFFFFF      # 32-bit gain
      wave[4] = length & 0xFFFFFFFF    # 32-bit length
      wave[5] = conf & 0xFFFF          # 16-bit config (w_conf)
