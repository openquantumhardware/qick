...
always @(posedge clk) begin
    if (~rstn) begin
        ...
        if ( trigger_state == 1'b1 ) begin
            ...
            if ( avg_state == 1'b1 && din_valid_i == 1'b1 ) begin
                // Accumulator counter.
                acc_i   <= acc_i + din_ii;
                acc_q   <= acc_q + din_qq;
            ...
            if ( (avg_state == 1'b1 && din_valid_i == 1'b1 && cnt > 1) || qout_state == 1'b1 ) begin
                // Rising edge counter.
                // cnt > 1 so we don't count edges that start before the readout window,
                // qout_state so we count edges that end in the last sample.
...
assign mem_di_o     = (photon_mode_r == 1'b0) ? {acc_q,acc_i} : acc_photon;
