module axi_slv_std_ro_v2 #(
    ...
    // Registers.
    output logic [31:0] FREQ_REG,
    output logic [31:0] PHASE_REG,
    output logic [15:0] NSAMP_REG,
    output logic [1:0] OUTSEL_REG,
    output logic MODE_REG,
    output logic WE_REG
);
    ...
    always_ff @(posedge aclk) begin
        if (~aresetn) begin
            ...
            if (axi_arready == 1 && arvalid == 1 && axi_rvalid == 0) begin
                axi_rvalid <= 1;
                axi_rresp <= 0;
            end else if (axi_rvalid == 1 && rready == 1) begin
                axi_rvalid <= 0;
            end
        end
    end
    ...
    // Output Registers.
    assign FREQ_REG = slv_reg0;
    assign PHASE_REG = slv_reg1;
    assign NSAMP_REG = slv_reg2[15:0];
    assign OUTSEL_REG = slv_reg3[1:0];
    assign MODE_REG = slv_reg4[0];
    assign WE_REG = slv_reg5[0];
