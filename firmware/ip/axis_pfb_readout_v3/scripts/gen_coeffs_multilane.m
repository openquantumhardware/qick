...
% The decomposition takes the first set of coefficients into the last
% filter, and so on. This is because the commutator model used inside the
% FIR filter compiler moves from down on every clock (model with z and
% decimators). Also, if the filter is multiplied by a complex exponential,
% this model ends up being followed by an FFT instead of a IFFT.
...
for jj=0:L-1
    ...
    idx = 2*jj;
    fn0 = sprintf('fir_%d.coe',idx);
    fn1 = sprintf('fir_%d.coe',idx+1);
