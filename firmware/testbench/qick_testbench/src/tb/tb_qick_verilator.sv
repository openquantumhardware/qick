...
rand_lite_master_addr8_data32_t axi_mst_qemu_agent;
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
...
// ============
   logic [5:0] SG_ADDR_START_ADDR   = 32'h40000000; // 0
...
// ============
   logic [7:0]   QEMU_DDS_BVAL_REG     = 4 * 0;
   logic [7:0]   QEMU_DDS_SLOPE_REG    = 4 * 1;
   logic [7:0]   QEMU_DDS_STEPS_REG    = 4 * 2;
   logic [7:0]   QEMU_DDS_WAIT_REG     = 4 * 3;
   logic [7:0]   QEMU_DDS_FREQ_REG     = 4 * 4;
   logic [7:0]   QEMU_IIR_C0_REG       = 4 * 5;
   logic [7:0]   QEMU_IIR_C1_REG       = 4 * 6;
   logic [7:0]   QEMU_IIR_G_REG        = 4 * 7;
   logic [7:0]   QEMU_OUTSEL_REG       = 4 * 8;
   logic [7:0]   QEMU_PUNCT_ID_REG     = 4 * 9;
   logic [7:0]   QEMU_ADDR_REG         = 4 * 10;
   logic [7:0]   QEMU_WE_REG           = 4 * 11;
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
...
// ============
   ...
   AXI_LITE_DV #(
      .AXI_ADDR_WIDTH ( 8       ),
      .AXI_DATA_WIDTH ( 32      )
   ) axi_mst_qemu_dv_IF (s_ps_dma_aclk);
   `AXI_LITE_ASSIGN(axi_mst_qemu_IF, axi_mst_qemu_dv_IF)

   assign s_axi_qemu_araddr        = axi_mst_qemu_IF.ar_addr  ; /* QEMU  input */
   assign s_axi_qemu_arprot        = axi_mst_qemu_IF.ar_prot  ; /* QEMU  input */
   assign s_axi_qemu_arvalid       = axi_mst_qemu_IF.ar_valid ; /* QEMU  input */
   assign axi_mst_qemu_IF.ar_ready = s_axi_qemu_arready       ; /* QEMU output */

   assign s_axi_qemu_awaddr        = axi_mst_qemu_IF.aw_addr  ; /* QEMU  input */
   assign s_axi_qemu_awprot        = axi_mst_qemu_IF.aw_prot  ; /* QEMU  input */
   assign s_axi_qemu_awvalid       = axi_mst_qemu_IF.aw_valid ; /* QEMU  input */
   assign axi_mst_qemu_IF.aw_ready = s_axi_qemu_awready       ; /* QEMU output */

   assign axi_mst_qemu_IF.b_resp   = s_axi_qemu_bresp         ; /* QEMU output */
   assign axi_mst_qemu_IF.b_valid  = s_axi_qemu_bvalid        ; /* QEMU output */
   assign s_axi_qemu_bready        = axi_mst_qemu_IF.b_ready  ; /* QEMU  input */

   assign axi_mst_qemu_IF.r_resp   = s_axi_qemu_rresp         ; /* QEMU output */
   assign axi_mst_qemu_IF.r_valid  = s_axi_qemu_rvalid        ; /* QEMU output */
   assign axi_mst_qemu_IF.r_data   = s_axi_qemu_rdata         ; /* QEMU output */
   assign s_axi_qemu_rready        = axi_mst_qemu_IF.r_ready  ; /* QEMU  input */

   assign s_axi_qemu_wdata         = axi_mst_qemu_IF.w_data   ; /* QEMU  input */
   assign s_axi_qemu_wstrb         = axi_mst_qemu_IF.w_strb   ; /* QEMU  input */
   assign s_axi_qemu_wvalid        = axi_mst_qemu_IF.w_valid  ; /* QEMU  input */
   assign axi_mst_qemu_IF.w_ready  = s_axi_qemu_wready        ; /* QEMU output */
...
// ============
   axi_mst_tproc_agent.write(PORT_AXI, prot, DATA_AXI, 8'hFF, resp);
...
// ============
   axi_mst_tproc_agent.read(ADDR_AXI, 0, DATA_RD, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
...
// ============
   axi_mst_avg_agent.write(BUF_START_REG, prot, data_wr, 8'hFF, resp);
   //                                                    ^~~~~~~~~~~~ write strobe set to all ones
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_avg_agent.write(BUF_LEN_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_avg_agent.write(AVG_START_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_avg_agent.write(AVG_LEN_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_avg_agent.write(BUF_DR_LEN_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// <<<<<<<<<<<< XILINX VIP PACKAGES
   // axi_mst_avg_agent.AXI4LITE_WRITE_BURST(BUF_DR_START_REG, prot, data_wr, resp);
// ============
   axi_mst_avg_agent.write(BUF_DR_START_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_avg_agent.write(AVG_DR_LEN_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_avg_agent.write(AVG_DR_START_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_qemu_agent.write(QEMU_DDS_FREQ_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_qemu_agent.write(QEMU_IIR_C0_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_qemu_agent.write(QEMU_IIR_C1_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_qemu_agent.write(QEMU_IIR_G_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// ============
   axi_mst_qemu_agent.write(QEMU_WE_REG, prot, data_wr, 8'hFF, resp);
// >>>>>>>>>>>> PULP PLATFORM AXI VIP
   #100ns;
...
// TODO: replace dac/adc model with the one produced by the HMC team 
