...
def read_dac_avtt():
    ...
    elif board in ['RFSoC4x2', 'RFSoC2x4']:
        devnum = 0x47
        busnum = 0
...
def control_fan(mode):
    ...
    if board == 'ZCU111':
        ...
        raise RuntimeError("RFSoC4x2/2x4 does not support fan control (it's always on auto)")
    else:
        raise RuntimeError("board %s is not recognized" % (board))
...
def set_dac_avtt(val):
    ...
    if board == 'ZCU111':
        ...
        raise RuntimeError("RFSoC4x2/2x4 does not support DAC_AVTT control (it's always on 2.5 V)")
    else:
        raise RuntimeError("board %s is not recognized" % (board))
